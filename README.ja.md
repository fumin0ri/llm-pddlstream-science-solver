...
このリポジトリは、LLM と `PDDLStream` を組み合わせた科学問題解法に関する私の卒業研究に対応する実装です。卒論本文は日本語で執筆しましたが、このリポジトリではより多くの人が実装と評価を理解しやすいよう、英語 README も併記しています。

就職活動用のポートフォリオとして見る場合、特に見てほしい点は次の通りです。

- 一発の prompting に頼らない多段階 LLM パイプラインを設計したこと
- 自然言語から実行可能な記号的成果物を生成したこと
- `PDDLStream` と `Fast Downward` を end-to-end の評価ループに統合したこと
- 生成プログラムに対する自動採点と再現実行の仕組みを整えたこと
