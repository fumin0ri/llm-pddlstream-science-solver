...
> エタン気体について、`n = 10.0 mol`, `V = 4.860 dm^3`, `T = 27°C` が与えられている。圧力を `atm` で求めよ。

このシステムは、いきなり最終解答を返すのではなく、中間成果物を段階的に生成します。

- 初期事実
  `temperature_c(27.0)`, `moles(10.0)`, `volume(4.860)`
- stream 計画
  `temp_to_K`: 摂氏をケルビンへ変換
  `pressure_from_nRT`: 理想気体の状態方程式から圧力を計算
  `final_answer`: 計算結果を最終解答として取り出す
- 実行結果
  `predicted_answer = 50.68`
  `expected_answer = 50.7`
  `correct = true`
